...
public:
	Session(boost::asio::io_context& io_service) :
		m_Socket(io_service), m_ReceiveBuffer({0,})
	{
	}
...
private:
	...
	{}
	void handle_recive(const boost::system::error_code& error, std::size_t bytes_transferred);
...
void Session::PostReceive()
{
	...
	(
		...
		boost::bind(&Session::handle_recive, this,
			boost::asio::placeholders::error,
			boost::asio::placeholders::bytes_transferred)
	);
}
void Session::handle_recive(const boost::system::error_code& error, std::size_t bytes_transferred)
{
	if (error)
	{
...
TCP_Server::TCP_Server(boost::asio::io_context& io_service) : m_acceptor(io_service,
